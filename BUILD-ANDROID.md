...
js/portrait.js  портрет: обрезка, сжатие, хранение в досье
...
android/        Android-проект Capacitor (о нём весь этот файл)
```

## Проверки
Перед выпуском гоняются три прогона. Сервер скрипты поднимают сами,
запускать рядом ничего не нужно. Аудит и оба прогона идут в CI до сборки:
...
node scripts/check-features.mjs  103 проверки живых функций всех модулей
