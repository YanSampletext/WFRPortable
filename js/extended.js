...
//
// Складываются уровни успеха как выпали, вместе с отрицательными, но сумма
// не опускается ниже 0 — так в книге (с. 118). Каждая
...
(function () {
  ...
  window.extRoll = function (id, mod) {
    ...
    var o = testOutcome(target, d), sl = o.sl;

    // Книга, с. 118: SL складываются как выпали, но если сумма падает ниже 0,
    // следующий бросок начинают с 0.
    e.acc = Math.max(0, (e.acc || 0) + sl);
    e.tries = e.tries || [];
  ...
  window.extBlockHtml = function () {
    ...
    var h = '<div class="panel" style="margin-bottom:14px;">' +
      ...
      'попытка — называет мастер. Складываются уровни успеха как выпали, вместе с отрицательными; ниже нуля сумма не опускается.</p>';
    if (!all.length) {
